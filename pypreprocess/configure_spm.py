...
_SPM_DEFAULTS['spm_versions'] = [12, 8]
...
def _get_defaults(template_name, version_nb=None, templates_dict=None):
    if templates_dict is None:
        templates_dict = _SPM_DEFAULTS
    if version_nb is None:
        version_nb = templates_dict['spm_versions']
    else:
...
def _get_exported(template_name, version_nb=None, templates_dict=None):
    env_vars = _get_defaults(template_name, version_nb, templates_dict)
...
def _find_or_warn(loc, check, msg=None, warner=_logger.warn, recursive=False):
    ...
    warner: callable, optional (default=configure_spm._logger.warn)
...
def _find_or_warn_in_seq(loc_seq, check, msg=None,
                         warner=_logger.warn, recursive=False):
...
class _IsValidMCR(object):
    ...
    def __init__(self, cli_spm_dir, config_spm_dir, defaults):
        ...
        in which to look if cli and config fail. Must provide the same keys
        and the same semantics as configure_spm._SPM_DEFAULTS
    ...
    def __call__(self, spm_mcr):
        ...
        if not _ACCEPT_SPM_MCR_WITH_UNKNOWN_VERSION and mcr_version is None:
            _logger.warn('_ACCEPT_SPM_MCR_WITH_UNKNOWN_VERSION is '
                         'set to False: rejecting "{}"'.format(spm_mcr))
            return False
        spm_dir_envs = _get_exported('spm_dir_env_template',
                                     mcr_version, self.defaults_)
...
def _find_spm_mcr_and_spm_dir(cli_spm_mcr, config_spm_mcr,
    ...
    if spm_mcr is None:
        _logger.warn('failed to find SPM MCR or SPM directory')
        return None
...
def _find_matlab_exec_and_spm_dir(
    ...
    if matlab_exec is None:
        _logger.warn('failed to find Matlab executable file')
        return None
    ...
    _logger.warn('failed to find SPM directory')
    return None
