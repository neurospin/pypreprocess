# *- encoding: utf-8 -*-
"""
pypreprocess version, required package versions, and utilities for checking
"""
...
_PYPREPROCESS_INSTALL_MSG = 'See %s for installation information.' % (
    'http://pypreprocess.github.io/introduction.html#installation')
...
def _import_module_with_version_check(
    ...
    try:
        ...
        user_friendly_info = ('Module "{0}" could not be found. {1}').format(
            module_name,
            install_info or 'Please install it properly to use pypreprocess.')
    ...
    if version_too_old:
        message = (
            ...
            'is required to use pypreprocess. {module_version} was found. '
...
def _check_module_dependencies(is_pypreprocess_installing=False):
    """Throw an exception if pypreprocess dependencies are not installed.

    Parameters
    ----------
    is_pypreprocess_installing: boolean
    ...
    for (module_name, module_metadata) in REQUIRED_MODULE_METADATA:
        if not (is_pypreprocess_installing and
