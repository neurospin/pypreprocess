...
:Synopsis: SPM use-case for preprocessing haxby dataset (this is just a quick-and-dirty POC)
...
def fetch_nyu_data_offline(nyu_rest_dir, session_ids=["session1"], subject_ids=["sub05676"]):
    """
    ...
    for session_id in session_ids:
        ...
        for subject_id in subject_ids:
            ...
            try:
                anonymized_image = glob.glob("%s/anat/mprage_anonymized.nii"%subject_dir)[0]
                skullstripped_image = glob.glob("%s/anat/mprage_skullstripped.nii"%subject_dir)[0]
                fmri_image = glob.glob("%s/func/lfo.nii"%subject_dir)[0]
            except IndexError:
                continue

            sessions[session_id][subject_id] = dict()
            sessions[session_id][subject_id]['skullstripped_anat'] = skullstripped_image
            sessions[session_id][subject_id]['anonymized_anat'] = anonymized_image
            sessions[session_id][subject_id]['func'] = fmri_image
